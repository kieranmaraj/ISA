// Mubu SOM code ported to Node from catart-mubu-master's descriptor_som.js
...
// Max function handlers
const handlers = {};
handlers.clear = clear;
handlers.setMapSize = setMapSize;
handlers.setInitialAlpha = setInitialAlpha;
handlers.setLearningRateType = setLearningRateType;
handlers.setMagnificationM = setMagnificationM;
handlers.setTrainingLength = setTrainingLength;
handlers.bufferindex = bufferindex;
handlers.appendData = appendData;
handlers.createSOM = createSOM;
...
function setMapSize(...mapSizeArgs)
{
  if (mapSizeArgs.length)
  {
    mapSize = mapSizeArgs;
    neuronCount = mapSize[0] * mapSize[1];
...
function setInitialAlpha(a){
    initialAlpha = a;
}

function setLearningRateType(l){
    learningRateType = l;
}

function setMagnificationM(m){
    magnificationM = m;
}

function setRange(...rangeArgs)
{
  var range = rangeArgs;
...
function setTrainingLength(trainLength)
{
  trainingLength = trainLength;
}

// bufferindex message must be sent after clear, before sequence of appendData
function bufferindex(bufIndex)
{
    var buffer = [bufIndex];
...
function appendData(...dataFrame)
{
  var sampleData = dataFrame;
...
//   train.repeat();
//   train = setInterval(training, 0.01);
  training();
}

function training()
{
  while(t < trainingLength)
  {
    trainingStep(t, trainingLength, rStep, alpha, winTimeStamp);
    // Progress percentage 
    Max.outlet("/progress " + String(math.floor(100 * (t / trainingLength))));
    t++;
  }
...
function outputDataCoordinatesOnMap()
{
  ...
  var vecSomCoords = bestMatches.map(function (vector) {
      var bmu = vector[0];
  ...
  console.log('Neuron occupation ('+ numempty +' empty):\n');
  // for all rows (upside down): print occupation
  for (var row = mapSize[1] - 1; row >= 0; row--)
  {
    // console.log(neuronOccupation.slice(row * mapSize[0], (row + 1) * mapSize[0]) +'\n');
  }

  // add random offset around neuron position to make classified vectors visible
  var vecSomCoordsX = vecSomCoords.map(function (vector) {
    return vector[0] + (math.random(randomRange[0], randomRange[1]));
  });
  var vecSomCoordsY = vecSomCoords.map(function (vector) {
  ...
  for (var i = 0; i < bufferid.length; i++)
  {
      ...
      Max.outlet("/currentBuffer " + String(bufferindex));

      // output sublist
      Max.outlet("/y " + String(vecSomCoordsY.slice(start, end)));
      Max.outlet("/x " + String(vecSomCoordsX.slice(start, end)));
  }
} // outputDataCoordinatesOnMap